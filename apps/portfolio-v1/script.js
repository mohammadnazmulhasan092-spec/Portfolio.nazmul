...
// Global version-switch listener: if the admin publishes Portfolio V2 (or
// rolls back) while someone is already viewing V1, this redirects them to
// the newly-active version immediately — no manual refresh needed. Runs
// once per page since script.js is loaded on every V1 portfolio page.
watchVersionSwitch('portfolio', 'v1');

// ============================================
// DYNAMIC GOOGLE FONTS LOADING
// ============================================
// The <head> only requests the 3 families the design always needs
// (Orbitron, Share Tech Mono, JetBrains Mono — see index.html). Every other
// family below is one the admin's Font Edit picker can choose as a
// site-wide override; only the one actually selected needs to be fetched,
// so it's loaded on demand here instead of unconditionally on every page
// load. Query strings match apps/admin-v1/admin.html's FULL_FONT_QUERY
// exactly — keep both in sync if the picker's option list changes.
const GOOGLE_FONT_QUERY = {
  ...
  'Russo One': 'family=Russo+One',
  'Dancing Script': 'family=Dancing+Script:wght@500;700',
  'Italianno': 'family=Italianno',
  'Montserrat': 'family=Montserrat:wght@400;600;700;900',
  'Poppins': 'family=Poppins:wght@400;600;700;900',
  'League Spartan': 'family=League+Spartan:wght@400;700;900',
  'Anton': 'family=Anton',
  'Bebas Neue': 'family=Bebas+Neue',
  'Archivo Black': 'family=Archivo+Black',
  'Raleway': 'family=Raleway:wght@400;700;900',
  'Cinzel': 'family=Cinzel:wght@400;700;900',
  'Cinzel Decorative': 'family=Cinzel+Decorative:wght@400;700;900',
  'Cormorant Garamond': 'family=Cormorant+Garamond:wght@400;700',
  'Playfair Display': 'family=Playfair+Display:wght@400;700;900',
  'Bodoni Moda': 'family=Bodoni+Moda:wght@400;700;900',
  'Pacifico': 'family=Pacifico',
  'Lobster': 'family=Lobster',
  'Satisfy': 'family=Satisfy',
  'Allura': 'family=Allura',
  'Great Vibes': 'family=Great+Vibes',
  'Kaushan Script': 'family=Kaushan+Script',
  'Sacramento': 'family=Sacramento',
  'Parisienne': 'family=Parisienne',
  'Alex Brush': 'family=Alex+Brush',
  'Yellowtail': 'family=Yellowtail',
  'Merienda': 'family=Merienda:wght@400;700;900',
  'Courgette': 'family=Courgette',
  'Marck Script': 'family=Marck+Script',
  'Arizonia': 'family=Arizonia',
  'Tangerine': 'family=Tangerine:wght@400;700',
  'Grand Hotel': 'family=Grand+Hotel',
};
const loadedFonts = new Set(['Orbitron', 'Share Tech Mono', 'JetBrains Mono']); // already in every page's <head>
// Given a CSS font-family stack (e.g. "'Pacifico', 'Brusher', cursive"),
// finds the first quoted name that's a real loadable Google Font and
// injects a <link> for it, if not already loaded.
function ensureGoogleFont(fontFamilyStack) {
  const names = (fontFamilyStack.match(/'([^']+)'/g) || []).map(s => s.slice(1, -1));
  const name = names.find(n => GOOGLE_FONT_QUERY[n] && !loadedFonts.has(n));
  if (!name) return;
  loadedFonts.add(name);
  const link = document.createElement('link');
  link.rel = 'stylesheet';
  link.href = `https://fonts.googleapis.com/css2?${GOOGLE_FONT_QUERY[name]}&display=swap`;
...
// (NOTE: the old basic loading-screen renderer was removed — it's no
// longer called anywhere. initLoaderPremium() further below is the loader
// actually used on index.html, and now reuses the same #boot-access-granted
// splash logic this function used to have.)
...
// (NOTE: the old basic "LIVE FEED" renderer was removed — it was dead code,
// never called anywhere; initLiveFeedPremium() below is the one actually used.)
...
export async function loadHomeContent() {
  ...
  recordVisit();
  // NOTE: the visitor panel itself is rendered exclusively by
  // loadVisitorPanelPremium() (see PREMIUM UPGRADE LAYER below). A basic
  // duplicate renderer used to run here too and race with the premium one
  // on the same #visitor-panel element — removed to fix that bug.
}
...
document.addEventListener('DOMContentLoaded', () => {
  // Skip on admin.html — it calls initCursor()/initGlitchEffects() itself
  // from initDashboard() after login, and doubling them up here wasted
  // resources (duplicate mousemove listeners, duplicate glitch loops).
  if (document.body.classList.contains('admin-body') || document.getElementById('dashboard-page')) return;
  // initLoader is replaced by the premium version below (see PREMIUM UPGRADE LAYER)
  initCursor();
  initNavUnderline();
  initGlitchEffects();
  initMobileMenu();
...
export function initSiteMeta() {
  ...
  // The label preserves each page's own descriptive title (e.g. "GALLERY —")
  // so a custom site title doesn't collapse V1's 5 distinct page titles into
  // one flat string. Empty label (index.html) means no prefix.
  const label = titleEl ? (titleEl.dataset.pageLabel || '') : '';
  subscribeSetting('site_meta', (d) => {
    d = d || {};
    // No stored title yet → leave the existing hardcoded <title> alone
    // (backward compatible default for databases that predate this setting).
    if (d.title) document.title = label ? `${label} — ${d.title}` : d.title;
...
document.addEventListener('DOMContentLoaded', () => {
  // IMPORTANT: initAppearance() applies the admin-configured site-wide
  // text color / brightness / font-size / font-family to (almost) every
  // element on the page. It must NOT run on admin.html — otherwise an
  // admin-chosen color/font could make the admin dashboard itself
  // unreadable or unusable. (This was a confirmed bug — see audit report.)
  if (document.body.classList.contains('admin-body') || document.getElementById('dashboard-page')) return;
  try { initAppearance(); } catch {}
  try { initFooterLogo(); } catch {}
  try { initSiteMeta(); } catch {}
...
export function initLightbox() {
  ...
  if (!box) {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Tab') { e.preventDefault(); box.querySelector('.lightbox-close').focus(); }
    });
  }
  ...
  imgs.forEach(img => {
    ...
    if (!img.hasAttribute('aria-label')) img.setAttribute('aria-label', 'View image: ' + (img.alt || 'untitled'));
    const open = () => {
...
// ---------- LIVE FEED (user-spec messages) ----------
// NOTE: this reads `cms.live_feed` — like initLoaderPremium(), `cms` is
// only read inside the function body and this is only ever called after
// the module finishes evaluating, so a single definition is enough (the
// old code duplicated this function for the same non-issue — see the
// audit report).
export function initLiveFeedPremium() {
  const container = document.getElementById('live-feed-content');
  if (!container) return;
  // Generation token: invalidates any previously-running loop from an
  // earlier call to this function so only one loop is ever active at a
  // time (fixes two concurrent loops garbling the feed — see audit report).
  const myGen = (parseInt(container.dataset.feedGen || '0', 10) + 1);
  container.dataset.feedGen = String(myGen);
  const isCurrent = () => container.dataset.feedGen === String(myGen);

  container.innerHTML = '';
  const messages = (typeof cms !== 'undefined' && Array.isArray(cms.live_feed) && cms.live_feed.length)
    ? cms.live_feed
  ...
  (function loop() {
    ...
    typeLine(messages[idx % messages.length], () => {
      idx++;
      if (isCurrent()) setTimeout(loop, 1500 + Math.random() * 800);
    });
  })();
}

// ---------- OVERRIDE: LOADER (user-spec text) ----------
// NOTE: this reads `cms.loader_lines` — `cms` (declared further below in
// this file) is only ever *read* here inside a function body, and this
// function is only ever called after the whole module has finished
// evaluating (from a DOMContentLoaded handler), so `cms` is always
// initialized by the time this runs. That means one implementation is
// enough — the old code duplicated this function (a hardcoded version here,
// then a second "patched" copy further down using cms.loader_lines) purely
// to work around an ordering problem that doesn't actually exist.
export function initLoaderPremium() {
  const screen = document.getElementById('loading-screen');
  if (!screen) return;
  // Use the loader's existing structure (boot-lines-container + the
  // dedicated ACCESS GRANTED splash + boot-bar) instead of wiping it with
  // innerHTML — the previous code destroyed #boot-access-granted every
  // time, so its neon-flicker "ACCESS GRANTED" splash never actually
  // appeared on the live site. Fixed here.
  const linesContainer = document.getElementById('boot-lines-container');
  const accessGranted = document.getElementById('boot-access-granted');
  if (!linesContainer) return; // no fallback to .boot-terminal — that would wipe #boot-access-granted again
  linesContainer.innerHTML = '';

  const lines = (typeof cms !== 'undefined' && Array.isArray(cms.loader_lines) && cms.loader_lines.length)
    ? cms.loader_lines
...
// (NOTE: the old v0 initHackerScene() background-monitor renderer was
// removed — initHackerSceneV13() further below fully replaces it.)
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  initStarsBg();
  // NOTE: the old v0 initHackerScene() call used to run here, left over
  // after initHackerSceneV13() was introduced to replace it. Both wrote to
  // the same #hs-screen element with separate un-cleared setInterval loops,
  // fighting each other forever. v13 (below, on DOMContentLoaded) is the
  // only one that should run — see audit report.
  initShootingStars();
  initIslamicHeader();
...
const CMS_DEFAULTS = {
  ...
  visual: { glow: 1, anim: 1, particles: 1, grid: 1 },
...
async function loadCmsAll() {
  ...
  await Promise.all(keys.map(async k => {
    try {
      const v = await (await import('../../shared/supabase.js')).getSetting('cms_' + k);
...
async function subscribeCms() {
  const { subscribeSetting } = await import('../../shared/supabase.js');
...
async function startLoaderWithCms() {
  try {
    const { getSetting } = await import('../../shared/supabase.js');
...
export function initHackerSceneV13() {
  ...
  computeRows();
  if (!screen.__v13ResizeBound) {
    screen.__v13ResizeBound = true;
...
/* Auto-init per-text styles + the background monitor animation on every page */
window.addEventListener('DOMContentLoaded', () => {
