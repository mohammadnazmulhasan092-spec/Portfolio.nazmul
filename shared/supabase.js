...
// Some browsers (notably Safari/other browsers in Private/Incognito mode
// with strict storage settings) throw a SecurityError the moment
// window.localStorage is touched — even just read access, not only writes.
// Since this file used to reference window.localStorage directly at
// module top-level, that throw would abort this entire module's
// evaluation, and every file that imports from it (script.js, admin.html,
// index.html, etc.) would fail too — the whole site would then get stuck
// on the loading screen forever. This wraps access in try/catch and falls
// back to an in-memory store so the Supabase client always initializes.
// ============================================
...
// ============================================
// Server-side (Storage bucket) limits are the real gate — see SETUP.sql's
// `allowed_mime_types` / `file_size_limit` on the portfolio-images bucket.
// These constants let every caller give the user immediate feedback
// instead of waiting on a network round-trip to find out the same thing.
export const ALLOWED_IMAGE_TYPES = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
export const MAX_IMAGE_BYTES = 5 * 1024 * 1024; // 5MB

export async function uploadImage(file, onProgress, folder = 'gallery') {
  if (!file || !(file instanceof Blob)) throw new Error('No file selected.');
  if (!ALLOWED_IMAGE_TYPES.includes(file.type)) {
    throw new Error('Unsupported file type — please choose a JPG, PNG, WEBP, or GIF image.');
  }
...
export async function recordVisit() {
  ...
  } catch (_) { /* sessionStorage blocked — just skip the dedupe, still record the visit */ }
  try {
