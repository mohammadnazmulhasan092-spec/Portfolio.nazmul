...
(async function bootCms() { if (document.body && document.body.classList.contains("admin-body")) return; if (document.getElementById("dashboard-page")) return;
  ...
  // Loader normally starts before this async CMS fetch resolves, so re-trigger
  // it here too — otherwise custom loader lines set in the admin panel never show.
  if (document.getElementById('loading-screen')) initLoaderPremium();
  initIslamicHeader();
  initHeroEnhancements();
  initLiveFeedPremium();
  initSkills();
